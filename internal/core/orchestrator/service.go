...
type Service struct {
	...
	goalInterlocks            map[string]*GoalInterlockLease
...
func (s *Service) ApplyAgentEvent(instanceID string, event agentproto.Event) []eventcontract.Event {
	...
	case agentproto.EventThreadRuntimeStatusUpdated:
		...
		return s.filterEventsForSurfaceVisibility(append(preface, s.applyGoalCommandResult(instanceID, event)...))
	...
	case agentproto.EventThreadGoalUpdated:
		if update := agentproto.NormalizeThreadGoalUpdate(event.ThreadGoal); update != nil && update.ExternalMutation {
			s.revokeGoalInterlockOnExternalMutation(instanceID, update.ThreadID)
		}
		return s.filterEventsForSurfaceVisibility(append(preface, s.applyThreadGoalUpdate(instanceID, event)...))
	case agentproto.EventThreadGoalCommandResult:
		return s.filterEventsForSurfaceVisibility(append(preface, s.applyGoalCommandResult(instanceID, event)...))
	...
	case agentproto.EventCapabilityStateUpdated:
		return s.filterEventsForSurfaceVisibility(append(preface, s.applyCapabilityStateUpdate(instanceID, event)...))
	...
	case agentproto.EventTurnCompleted:
		...
		if event.ThreadID != "" {
			events = append(events, s.pumpAndResumeGoalInterlock(instanceID, event.ThreadID)...)
		}
		events = append(events, compactEvents...)
		return s.filterEventsForSurfaceVisibility(events)
